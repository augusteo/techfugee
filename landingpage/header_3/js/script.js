(function($) {
	...
	var Appro = {
		...
		Form: function() {
			...
			$('.affa-form-contact input[type="text"], .affa-form-contact input[type="email"], .affa-form-contact textarea, .affa-form-signup input[type="text"], .affa-form-signup input[type="email"], .affa-form-signup input[type="password"], .affa-form-signup input[type="number"], .affa-form-signup select').on('focus keypress', function() {
				var $input = $(this);
						...
						success: function(msg) {
							if (parseInt(msg, 0) !== 0) {
								...
								if (msg_split[0] === 'success') {
									...
									$submit.removeAttr('disabled');
									$form.find('.submit-status').attr('class', 'submit-status success').html('<i class="ion ion-ios-checkmark-outline"></i> ' + msg_split[1]).fadeIn(300).delay(3000).fadeOut(300);
								} else {
									$name.removeAttr('disabled');
									$email.removeAttr('disabled');
									$submit.removeAttr('disabled');
									$form.find('.submit-status').attr('class', 'submit-status error').html('<i class="ion ion-ios-close-outline"></i> ' + msg_split[1]).fadeIn(300).delay(3000).fadeOut(300);
								}
							}
						}
					});
